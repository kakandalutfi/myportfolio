...
export function CreatorProfile() {
  return (
    <section id="profile" className="max-w-7xl mx-auto px-6 py-12 md:py-20 scroll-mt-32">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {stats.map((stat) => (
              <div 
                key={stat.label} 
                className="p-6 md:p-8 glass rounded-[1.5rem] md:rounded-[2rem] border border-border shadow-2xl bg-card/40 text-center hover:border-blue-500/20 transition-colors"
              >
                <div className="flex justify-center mb-2 md:mb-3 text-blue-600 dark:text-blue-400">
                  {stat.icon}
                </div>
                <h3 className="text-3xl md:text-4xl font-bold text-foreground mb-1 md:mb-2">{stat.value}</h3>
                <p className="text-xs md:text-base text-muted-foreground font-medium">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mb-16 md:mb-24 text-center">
        <h3 className="text-xl md:text-2xl font-bold text-foreground mb-8 md:mb-10">Client Feedback</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((test, index) => (
            <motion.div
              ...
              className="p-6 md:p-8 glass rounded-[1.5rem] md:rounded-[2rem] border border-border bg-card/40 relative flex flex-col justify-between hover:border-blue-500/20 shadow-premium text-left"
            >
              <div className="mb-6">
      ...
      <div className="glass rounded-[2rem] md:rounded-[3rem] p-8 md:p-20 text-center border border-border bg-card/40 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-blue-600/5 to-indigo-600/5" />
        <h3 className="text-xl md:text-4xl font-bold text-foreground mb-6 md:mb-8 relative z-10">
          Ready to elevate <span className="text-gradient">your digital presence?</span>
        </h3>
        <a 
          href="#contact"
          className="inline-flex items-center gap-2 md:gap-3 px-8 py-4 md:px-10 md:py-5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-full font-bold text-base md:text-lg hover:shadow-[0_10px_30px_rgba(59,130,246,0.3)] hover:-translate-y-1 transition-all relative z-10 cursor-pointer"
        >
          Let's Collaborate <ArrowRight size={18} className="md:w-5 md:h-5" />
        </a>
      </div>
    </section>
  );
